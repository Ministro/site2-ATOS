...
export default async function handler(req, res) {
  ...
  try {
    ...
    // A FK de game_premio_tokens.ganho_id exige que o ganho exista antes da reserva.
    // Para evitar token duplicado, criamos o ganho provisório e reservamos o token
    // com atualização condicional. Se outro jogador pegar o token primeiro, o ganho
    // provisório é removido e a tentativa continua com outro token.
    let escolhido = null;
    let tokenReservado = null;
    let ganhoId = null;
    const agora = new Date().toISOString();

    for (let tentativa = 0; tentativa < 8 && !tokenReservado; tentativa += 1) {
      escolhido = escolherPonderado(candidatos);
      if (!escolhido || !escolhido.tokens.length) break;

      const indice = Math.floor(Math.random() * escolhido.tokens.length);
      const token = escolhido.tokens.splice(indice, 1)[0];
      escolhido.peso = Math.min(
        Number(escolhido.premio.quantidade || 0),
        escolhido.tokens.length
      );

      ganhoId = crypto.randomUUID();
      ...
      try {
        const reservado = await supabase(
          `game_premio_tokens?id=eq.${encodeURIComponent(token.id)}&status=eq.disponivel&ganho_id=is.null`,
          {
            method: 'PATCH',
        ...
        if (reservado?.[0]) {
          tokenReservado = { ...token, ...reservado[0] };
          break;
        }
      } catch (erroReserva) {
        // Remove o ganho provisório antes de tentar outro token.
      }

      await supabase(`game_premios_ganhos?id=eq.${encodeURIComponent(ganhoId)}`, {
        method: 'DELETE',
        headers: { Prefer: 'return=minimal' }
      }).catch(() => {});
      ganhoId = null;
    ...
    if (!escolhido || !tokenReservado || !ganhoId) {
    ...
    try {
      ...
      // Reverte a reserva e remove o ganho se a finalização falhar.
      await supabase(`game_premio_tokens?id=eq.${encodeURIComponent(tokenReservado.id)}&ganho_id=eq.${encodeURIComponent(ganhoId)}`, {
        method: 'PATCH',
        headers: { Prefer: 'return=minimal' },
